...
const ShippingMethodTableData = ({
  ...
  ShippingMethod?: ShippingMethodTableDataProps[];
}) => {
  ...
  const renderShippingMethod = ShippingMethod.map((shippingMethod, index) => {
    ...
    return (
      <tr key={index}>
        ...
        {+selectedItems[0]  === shippingMethod.id && selectedItems.length < 2 ?
                    ...
                    <button onClick={DeletItem} className="text-black flex space-x-1 border font-semibold border-black px-5 py-1.5 hover:bg-red-600 hover:text-white hover:border-red-600 transition-all duration-300" style={{borderRadius:"5px"}}>
                      Delete 
                    ...
                    :
                    <button className="text-white bg-black px-5 py-1.5 hover:bg-black/85 transition-all duration-300" style={{borderRadius:"5px"}}>Edit</button>
